...
const Sidebar: React.FC<SidebarProps> = ({ onNavItemClick }) => {
  ...
  return (
    <aside className="w-64 h-full bg-gradient-to-b from-yellow-50 via-white to-yellow-100 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 text-gray-900 dark:text-gray-100 border-r-4 border-yellow-400 shadow-xl transition-colors duration-300 flex flex-col rounded-tr-3xl rounded-br-3xl overflow-hidden">
      {/* Logo or Title */}
      <div className="flex items-center justify-center py-8 mb-6 select-none">
        <span className="inline-flex items-center text-2xl font-extrabold tracking-tight text-yellow-500 dark:text-yellow-400">
          <PiggyBankIcon size={32} className="mr-2" />
          BudgetBoss
        </span>
      </div>
            ...
            {navItems.map((item) => (
              <li key={item.path}>
                <NavLink
                  ...
                  className={({ isActive }) => 
                    `flex items-center gap-3 px-5 py-3 rounded-lg text-base font-semibold transition-all duration-200 shadow-sm group
                    ${isActive
